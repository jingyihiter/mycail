# -*- coding: utf-8 -*-
"""
在大数据集上textcnn和han分别在前两个任务上保留了最好的模型
以这四个模型作模型融合，投票机制选择
"""
...
class Predictor(object):
    def __init__(self):
        ...
        tf.app.flags.DEFINE_string("ckpt_dir_han_accu", "./checkpoint_hans/checkpoint_accu/", "checkpoint location fpr the han model of accusation")
        tf.app.flags.DEFINE_string("ckpt_dir_han_law", "./checkpoint_hans/checkpoint_law/", "checkpoint location for the han model of law")
        ...
        tf.app.flags.DEFINE_string("model_han", "han", "name of model:han,c_gru,c_gru2,gru,text_cnn")
        ...
        # han model
        graph_han = tf.Graph().as_default()
        with graph_han:
            self.model_han = HierarchicalAttention(accusation_num_classes, article_num_classes, deathpenalty_num_classes,lifeimprisonment_num_classes,
                                    FLAGS.learning_rate, self.batch_size,FLAGS.decay_steps, FLAGS.decay_rate, FLAGS.sentence_len, FLAGS.num_sentences,vocab_size,
                                    FLAGS.embed_size_dpcnn, FLAGS.hidden_size_dpcnn,num_filters = FLAGS.num_filters, model = FLAGS.model_han, filter_sizes = filter_sizes,
                                    stride_length = stride_length)
            saver_big_accu = tf.train.Saver()
            sess_big_accu = tf.Session(config=config)
            saver_big_accu.restore(sess_big_accu, tf.train.latest_checkpoint(FLAGS.ckpt_dir_han_accu))
            self.sess_big_accu=sess_big_accu
            saver_big_law = tf.train.Saver()
            sess_big_law = tf.Session(config=config)
            saver_big_law.restore(sess_big_law, tf.train.latest_checkpoint(FLAGS.ckpt_dir_han_law))
            self.sess_big_law = sess_big_law

        self.FLAGS=FLAGS

    def vote(self, x1, x2, x3, x4):
        sum_n = 0
        if x1>=0.5:
            sum_n += 1
        if x2>=0.5:
