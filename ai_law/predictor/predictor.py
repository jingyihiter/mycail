...
class Predictor(object):
    def __init__(self):
        ...
        tf.app.flags.DEFINE_string("ckpt_dir_accu", "./task3/checkpoint_accu/","checkpoint location for the best model of accusation")
        tf.app.flags.DEFINE_string("ckpt_dir_law", "./task3/checkpoint_law/", "checkpoint location for the best model of law_article")
        tf.app.flags.DEFINE_string("ckpt_dir_imprision", "./task3/checkpoint_imprision", "checkpoint location for best model of imprisonment")
        ...
        with graph:
            ...
            saver_accu.restore(sess_accu, tf.train.latest_checkpoint(FLAGS.ckpt_dir_accu))
            ...
            # saver_imprison = tf.train.Saver()
            # sess_imp = tf.Session(config=config)
            # saver_imprison.restore(sess_imp, tf.train.latest_checkpoint(FLAGS.ckpt_dir_imprision))
            # self.sess_imp = sess_imp
    ...
    def predict_with_model_batch(self,contents):
        ...
        # _,_,logits_deathpenaltys,logits_lifeimprisonments,logits_imprisonments= self.sess_imp.run([model.logits_accusation,model.logits_article,model.logits_deathpenalty,model.logits_lifeimprisonment,model.logits_imprisonment],feed_dict)
