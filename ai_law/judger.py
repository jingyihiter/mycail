...
class Judger:
    ...
    def gen_score(self, arr):
        ...
        return (f_ + sumf * 1.0 / len(arr)) / 2.0,sumf * 1.0 / len(arr),f_

    # Generatue all scores
    def get_score(self, result):
        s1,s1_sumf,s1_f_ = self.gen_score(result[0])
        s2,s2_sumf,s2_f_ = self.gen_score(result[1])
        s3 = 1.0 * result[2]["score"] / result[2]["cnt"]
        return [[s1,s1_sumf,s1_f_], [s2,s2_sumf,s2_f_], s3]
    ...
    def evalute(self, truthdic, preddic):
        result = [[],[],{}]
        for a in range(0, self.task1_cnt): # 罪行
            result[0].append({"TP": 0, "FP": 0, "TN": 0, "FN": 0})
        for a in range(0, self.task2_cnt): # 法条
            result[1].append({"TP": 0, "FP": 0, "TN": 0, "FN": 0})
        result[2] = {"cnt": 0, "score": 0} # 刑期
        for key, value in preddic.items():
            tru_value = truthdic[key]
            result = self.gen_new_result(result, tru_value, value)
        return self.get_score(result)
