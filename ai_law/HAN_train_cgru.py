...
# 用于在C-gru模型上大数据集上的结果，可能用于之后的模型融合
#import sys
#reload(sys)
...
tf.app.flags.DEFINE_string("traning_data_file","./../big_data/train_set.json","path of traning data.")
tf.app.flags.DEFINE_string("valid_data_file","./../big_data/valid_set.json","path of validation data.")
tf.app.flags.DEFINE_string("test_data_path","./../big_data/test_set.json","path of validation data.")
tf.app.flags.DEFINE_string("predict_path","./predictor","path of traning data.")
tf.app.flags.DEFINE_string("stopwords_file", "./data/stopword.txt", "path of stopword file")
tf.app.flags.DEFINE_string("ckpt_dir_cgru","./checkpoint_cgrus/checkpoint/","checkpoint location for the model") #save to here, so make it easy to upload for test
tf.app.flags.DEFINE_string("ckpt_dir_accu","./checkpoint_cgrus/checkpoint_accu/","checkpoint location for the model") 
tf.app.flags.DEFINE_string("ckpt_dir_law","./checkpoint_cgrus/checkpoint_law/","checkpoint location for the model") 
tf.app.flags.DEFINE_string("ckpt_dir_imprision","./checkpoint_cgrus/checkpoint_imprision/","checkpoint location for the model")

tf.app.flags.DEFINE_string("log_path", "./ai_law_cgru.log", "log path location")
...
tf.app.flags.DEFINE_string("model","c_gru","name of model:han,text_cnn,dp_cnn,c_gru,c_gru2,gru,pooling")
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                if start!=0 and start%(2000*FLAGS.batch_size)==0: # eval every 400 steps.
                    ...
                    if accasation_score>accasation_score_best:
                        ...
                        logger.info("going to save check point.")
                        saver.save(sess, save_path, global_step=epoch)
                        accasation_score_best=accasation_score
                    if law_score_best < article_score:
                    ...
                    if score_penalty >  imprison_score_best:
                        save_path = FLAGS.ckpt_dir_imprision + "model.ckpt" #TODO temp remove==>only save checkpoint for each epoch once.
                        logger.info("going to save check point.")
                        saver.save(sess, save_path, global_step=epoch)
                        imprison_score_best = score_penalty
                    logger.info("Epoch:{} Accasation{} Article{} Penalty{}".format(epoch, accasation_score_best, law_score_best, imprison_score_best))
            #epoch increment
...
def assign_pretrained_word_embedding(sess,vocabulary_index2word,vocab_size,model,word2vec_model_path,embedding_instance):
    ...
    print("====>>>>word. exists embedding:{} ;word not exist embedding:{}".format(count_exist, count_not_exist))
