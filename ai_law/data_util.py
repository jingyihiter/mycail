...
def load_data_multilabel(traning_data_path,valid_data_path,test_data_path, stopword_file,vocab_word2index, accusation_label2index,article_label2index,
    ...
    cache_file =cache_data_dir+"/"+'train_valid_test.hdf5'
    print("cache_path:",cache_file,"train_valid_test_file_exists:",os.path.exists(cache_file))
    if os.path.exists(cache_file):
        dataset_f = h5py.File(cache_file, 'r')
        print("going to load cache file from file system and return")
        train_data = dataset_f['train']
        valid_data = dataset_f['valid']
        test_data = dataset_f['test']
    ...
    stopwords = list()
    with open(stopword_file, "r", encoding="utf-8") as tf:
        for line in tf:
            stopwords.append(line.strip('\n'))
    if test_mode:
        train_lines=train_lines[0:1000]
    # 3. transform to train/valid data to standardized format
    train=transform_data_to_index(train_lines, vocab_word2index, accusation_label2index, article_label2index,deathpenalty_label2index, lifeimprisonment_label2index, sentence_len,'train',stopwords,name_scope)
    valid=transform_data_to_index(valid_lines, vocab_word2index, accusation_label2index, article_label2index,deathpenalty_label2index, lifeimprisonment_label2index, sentence_len,'valid',stopwords,name_scope)
    test=transform_data_to_index(test_lines, vocab_word2index, accusation_label2index, article_label2index,deathpenalty_label2index, lifeimprisonment_label2index, sentence_len,'test',stopwords,name_scope)

    # 4. save to file system if vocabulary of words not exists
    # #not save
    # if not os.path.exists(cache_file):
    #     dataset_f = h5py.File(cache_file, 'w')
    #     print("going to dump train/valid/test data to file sytem.")
    #     dataset_f['train'] = train
    #     dataset_f['valid'] = valid
    #     dataset_f['test'] = test
    return train,valid,test

splitter=':'
num_mini_examples=5000#3500
def transform_data_to_index(lines,vocab_word2index,accusation_label2index,article_label2index,deathpenalty_label2index,lifeimprisonment_label2index,
                            sentence_len,data_type,stopwords,name_scope,reverse_flag=False):
    """
    ...
    for i, line in enumerate(lines):
        ...
        input_list = json_string['seg_fact']
        # input_list = [word for word in inputlist if word not in stopwords] # 先不去停用词
